...
- 출력 키: `summary`, `correctionPoints`, `recommendation`, `exampleAnswer`, `rulebookEvidence`
- `summary`: 1~2문장
- `correctionPoints`: 정확히 3개
    - `Grammar` 1개
    - `Expression` 1개
    - `Logic` 1개
- `recommendation`: 정확히 3개
    - `Filler` 1개 (독립 항목)
    - `Adjective` 1개 (독립 항목)
    - `Adverb` 1개 (독립 항목)
- `correctionPoints`, `recommendation`은 반드시 JSON 배열(`[]`) 타입이어야 하며, 객체(`{}`)/문자열/null 금지
- 순서 고정:
    - `correctionPoints`: `Grammar`, `Expression`, `Logic`
    - `recommendation`: `Filler`, `Adjective`, `Adverb`
- 각 항목 형식: `Category: 설명`
...
- `correctionPoints` 또는 `recommendation` 스키마 위반(개수/카테고리 누락/배열 아님) 시 서버가 기본값으로 보정한다.
