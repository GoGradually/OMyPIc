...
- [ ] 질문 리스트 도메인/퍼시스턴스/API 제거
- [ ] 질문그룹 Aggregate 도입 (`/api/question-groups` 중심)
- [ ] 질문그룹:태그를 n:m 구조로 저장 (`tags[]`)
- [ ] 질문 항목에 `questionType(nullable)` 필드 추가 (CRUD/UI round-trip)
- [ ] 태그 정규화 규칙 적용: `trim + lowercase`
- [ ] 태그 통계 단일 API 추가: `GET /api/question-groups/tags/stats`
- [ ] 통계 응답 스키마 고정: `[{tag, groupCount, selectable}]`
- [ ] 통계 정렬 규칙 고정: `tag` 오름차순
- [ ] `groupCount==0`이면 `selectable=false`

### 6-2. 모드 설정 계약 변경(listId 제거)

- [ ] `PUT /api/modes` 요청에서 `listId` 제거
- [ ] `PUT /api/modes` 요청에 `selectedGroupTags(string[])` 필수 추가
- [ ] `selectedGroupTags` 빈 배열 요청 시 400 처리
- [ ] 무효 태그 포함 시 400 `INVALID_GROUP_TAGS` 처리
- [ ] 에러 응답 스키마 고정: `{code,message,invalidTags[]}`

### 6-3. 세션 출제/랜덤 규칙 고정

- [ ] 모드 적용 시 `selectedGroupTags` 기준 후보 그룹 계산
- [ ] 후보 그룹 순서는 세션 단위 1회 셔플(세션 내 고정)
- [ ] 그룹 내부 질문 순서는 등록 순서 유지
- [ ] 후보 그룹 없음/전체 소진 시 `NextQuestion.skipped=true`
- [ ] 종료 사유는 `QUESTION_EXHAUSTED` 단일 규칙 사용

### 6-4. 연속 모드 배치 단위 전환(질문 -> 그룹)

- [ ] `continuousBatchSize` 의미를 “완료된 질문그룹 수”로 재정의
- [ ] 배치 카운터를 `answeredQuestionCount`에서 `completedGroupCount`로 전환
- [ ] 그룹 완료 판정 규칙 구현: “다음 그룹 진입 또는 질문 소진 시점”
- [ ] 질문 소진 시 그룹 잔여 배치 1회 emit 규칙 구현
- [ ] realtime `feedback.final.policy`에서 `batchSize` 제거
- [ ] realtime `feedback.final.policy`에 `groupBatchSize` 추가
- [ ] 프론트 realtime 소비 로직을 `groupBatchSize` 기준으로 교체
- [ ] 프론트 문구를 “문항” -> “질문 그룹” 기준으로 교체

### 6-5. 룰북/질문타입 적용 범위 고정

- [ ] `questionType`은 질문 CRUD/UI 용도로만 사용
- [ ] `RulebookScope`는 기존 `MAIN/QUESTION` 유지
- [ ] `QUESTION_TYPE` scope 추가하지 않음
- [ ] `searchContextsForTurn`에 questionType 우선선택 로직 추가하지 않음
...
- [ ] 태그 정규화(`trim+lowercase`) 검증
- [ ] 세션 고정 셔플 재현성 검증
- [ ] selected tags 외 그룹 미출제 검증
- [ ] 후보 그룹 소진 시 `skipped + QUESTION_EXHAUSTED` 검증
- [ ] `completedGroupCount` 기준 배치 emit 검증
- [ ] 질문 소진 시 잔여 그룹 피드백 emit 검증

### 7-2. 백엔드 통합/컨트롤러

- [ ] `PUT /api/modes` 정상 케이스: `selectedGroupTags` 적용 검증
- [ ] `PUT /api/modes` 빈 태그: 400 + `INVALID_GROUP_TAGS` 검증
- [ ] `PUT /api/modes` 무효 태그: 400 + `invalidTags[]` 검증
- [ ] `GET /api/question-groups/tags/stats` 응답 스키마/정렬 검증
- [ ] 질문 CRUD/조회/next에서 `questionType` round-trip 검증
- [ ] realtime `feedback.final.policy.groupBatchSize` 검증

### 7-3. 프론트

- [ ] 리스트 선택 UI 제거 회귀 검증
- [ ] 태그 멀티선택 버튼 렌더링(통계 API 기반) 검증
- [ ] `selectable=false` 태그 disabled 검증
- [ ] 모드 적용 payload에 `selectedGroupTags[]` 포함 검증
- [ ] 빈 태그 선택 시 요청 차단/에러 표시 검증
- [ ] realtime policy 소비를 `groupBatchSize`로 전환 검증
- [ ] 요약/상태 문구가 “질문 그룹” 단위로 표시되는지 검증
