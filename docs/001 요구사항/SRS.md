...
- 질문 TTS(`role=question`)는 **동일 오디오를 총 2회 재생**한다.
- 질문 TTS 2회 재생 정책은 **1차 즉시 재생 + 3초 대기 후 2차 재생**이다.
- 질문 TTS 2차 재생은 **기존에 수신한 오디오를 재생**하며, 질문 반복을 위해 TTS를 재합성하지 않는다.
...
- AC-VS-01: 질문 `tts.audio(role=question)` 1건 수신 시 질문 오디오는 2회 재생된다.
- AC-VS-02: 2차 재생은 1차 재생 완료 후 3초 뒤 시작된다.
- AC-VS-03: `speechState`는 질문 2차 재생 완료 후 `READY_FOR_ANSWER`로 전이된다.

### 1.4 내 응답 다시듣기(섀도잉) 원칙

- 사용자는 직전 마지막 답변을 **`내 응답 재생` / `중지`** 버튼으로 다시 들을 수 있어야 한다.
- 재생 소스는 STT 텍스트 재합성이 아니라, **사용자 실제 답변 PCM16 녹음본**이어야 한다.
- 다시듣기 후보 오디오는 턴 종료 시점에 수집하고, `stt.final(turnId)`과 턴이 일치할 때만 확정한다.
- `stt.skipped(empty_transcript)` 턴은 다시듣기 확정 대상에서 제외한다.
- 세션 활성 중 다시듣기 시작 가능 구간은 `speechState === WAITING_TTS` 또는 `speechState === READY_FOR_ANSWER`로 제한한다.
- `speechState === CAPTURING_ANSWER`에서는 다시듣기를 시작할 수 없다.
- `READY_FOR_ANSWER`에서 다시듣기 재생이 시작되면, 재생이 끝날 때까지 마이크 캡처를 차단해야 한다.
- 다시듣기 재생 중 도착한 `tts.audio`는 즉시 재생하지 않고 대기 큐에 넣은 뒤, 다시듣기 종료 직후 순차 재생한다.
- 세션 종료 후에도 확정된 다시듣기 오디오가 있으면 재생 가능해야 한다.
- 새 세션 시작 시 이전 세션의 다시듣기 확정 오디오는 초기화해야 한다.
- 비활성 사유는 버튼 툴팁으로 제공한다.

#### 다시듣기 AC

- AC-RP-01: `stt.final` 완료 후 조건 충족 시 다시듣기 버튼이 활성화된다.
- AC-RP-02: `stt.skipped(empty_transcript)` 턴에서는 다시듣기 확정 오디오가 생성되지 않는다.
- AC-RP-03: 다시듣기 재생 중 도착한 `tts.audio`는 즉시 재생되지 않고 대기된다.
- AC-RP-04: 다시듣기 종료 후 대기된 TTS 오디오가 순서대로 재생된다.
- AC-RP-05: `READY_FOR_ANSWER`에서 다시듣기 재생 중에는 마이크 캡처가 시작되지 않는다.
- AC-RP-06: 세션 종료 후 다시듣기 가능 상태를 유지하고, 다음 세션 시작 시 초기화된다.
