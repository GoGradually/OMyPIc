...
class RealtimeVoiceWebSocketHandlerTest {
    ...
    void setUp() {
        ...
        lenient().when(webSocketSession.getId()).thenReturn("ws-1");
        lenient().when(webSocketSession.isOpen()).thenReturn(true);
        lenient().when(realtimeVoiceUseCase.open(any(), any())).thenReturn(realtimeVoiceSession);
    }
    ...
    void handleTextMessage_routesAudioAndControlEvents() throws Exception {
        ...
        handler.handleTextMessage(webSocketSession, new TextMessage("{\"type\":\"session.update\",\"data\":{\"feedbackLanguage\":\"en\"}}"));
        ...
        verify(realtimeVoiceSession).update(any());
    }
